...
function JourneyCard({ slug, title, image, tags, bgClass, imagePosition = 'object-center' }) {
  return (
...
export default function Journeys() {
  const [journeys] = useState(journeysData)
      ...
      <main className="space-y-10 pb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-semibold tracking-tight text-foreground">My Journey</h1>
            <p className="text-[15px] text-muted mt-2">Stories behind the milestones and achievements.</p>
          </div>
        </div>
        ...
        {!loading && error && (
          <div className="rounded-2xl border border-red-100 bg-red-50/80 px-5 py-4 text-[15px] text-red-800">
            <p className="font-medium">Could not load journeys.</p>
            <p className="text-red-700/90 mt-1">{error}</p>
          </div>
        )}

        {!loading && !error && journeys.length === 0 && (
          <p className="text-[15px] text-muted">No journeys yet.</p>
        )}
